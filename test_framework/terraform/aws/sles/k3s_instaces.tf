...
resource "null_resource" "rsync_kubeconfig_file" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "until([ -f /etc/rancher/k3s/k3s.yaml ] && [ `sudo /usr/local/bin/kubectl get node -o jsonpath='{.items[*].status.conditions}'  | grep -i -o 'type:Ready' | wc -l` -eq 4  ]); do echo \"waiting for k3s cluster nodes to be running\"; sleep 2; done"
    ]

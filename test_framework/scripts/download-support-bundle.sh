...
SUPPORT_BUNDLE_RESP=`kubectl exec -n longhorn-system svc/longhorn-frontend -- bash -c "${CURL_CMD}"`
echo ${SUPPORT_BUNDLE_RESP}
SUPPORT_BUNDLE_URL=`echo ${SUPPORT_BUNDLE_RESP} | jq -r '.links.self + "/" + .name'`

...
resource "rancher2_catalog_v2" "longhorn_repo" {
  cluster_id = "local"
  name = "longhorn-repo"
  git_repo = var.longhorn_git_repo
  git_branch = var.longhorn_git_branch
}
...
resource "rancher2_app_v2" "longhorn_app" {
  ...
  chart_name = "longhorn"
  #chart_version = "100.1.2+up1.2.4" # if chart_version is not specified, the latest version would be installed
  values = <<-EOF
privateRegistry:
  createSecret: true

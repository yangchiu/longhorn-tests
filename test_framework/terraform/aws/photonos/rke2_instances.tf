...
resource "aws_instance" "lh_aws_instance_controlplane_rke2" {
 depends_on = [
  ...
  ami           = data.aws_ami.aws_ami_photonos.id
...
resource "null_resource" "rsync_kubeconfig_file_rke2" {
  ...
  provisioner "local-exec" {
    command = "rsync -aPvz --rsync-path=\"sudo rsync\" -e \"ssh -o StrictHostKeyChecking=no -l root -i ${var.aws_ssh_private_key_file_path}\" ${aws_eip.lh_aws_eip_controlplane[0].public_ip}:/etc/rancher/rke2/rke2.yaml .  && sed -i 's#https://127.0.0.1:6443#https://${aws_eip.lh_aws_eip_controlplane[0].public_ip}:6443#' rke2.yaml" 
  }
}
